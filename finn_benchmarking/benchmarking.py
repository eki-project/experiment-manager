...
def main():
    ...
    try:
        ...
        print("Launched as job array (Array ID: %d, Task ID: %d, Task count: %d)"%(array_id, task_id, task_count))
    except KeyError:
        array_id = job_id
        task_id = 0
        task_count = 1
    ...
    log_path = os.path.join(results_dir, "task_%d.json"%(task_id))

    # Select benchmark
    print("Running benchmark %s"%(sys.argv[1]))
    if sys.argv[1] == "rtl_swg":
        bench = bench_rtl_swg
    ...
    config_path = os.path.join(os.path.dirname(__file__), sys.argv[2])
    ...
    for param_set in config:
        param_set_expanded = list(dict(zip(param_set.keys(), x)) for x in itertools.product(*param_set.values()))
        config_expanded.extend(param_set_expanded)

    # Save config (only first job of array)
    if task_id == 0:
        with open(os.path.join(experiment_dir, "bench_config.json"), "w") as f:
                json.dump(config, f, indent=2)
        with open(os.path.join(experiment_dir, "bench_config_exp.json"), "w") as f:
    ...
    for run, run_id in enumerate(selected_runs):
        print("Starting run %d/%d (id %d of %d total runs)"%(run+1, len(selected_runs), run_id, total_runs))
