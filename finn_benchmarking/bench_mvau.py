...
def make_single_mvau_model(W, numInputVectors, pe, simd, m, wdt, idt, odt, T=None, tdt=None, 
    mem_mode="const", ram_style="auto", ram_style_thresholds="auto"):
    mw = W.shape[0]
    mh = W.shape[1]
...
def bench_mvau(params, task_id, run_id, results_dir):
    ...
    if sparsity_type == "none":
        ...
        if sparsity_type == "unstructured":
            idx = np.random.choice(mw*mh, size=int(params["sparsity_amount"]*mw*mh), replace=False)
            W = np.reshape(W, -1)
            W[idx] = 0.0
            ...
            if params["sparsity_amount"] == 0.25:
                ...
                idx_mw = np.concatenate((np.arange(0, mw, step=4),np.arange(1, mw, step=4),np.arange(2, mw, step=4)))
            else:
            ...
            if params["sparsity_amount"] == 0.25:
                ...
                idx_mh = np.concatenate((np.arange(0, mh, step=4),np.arange(1, mh, step=4),np.arange(2, mh, step=4)))
            else:
                print("regular sparsity only applicable for amount 0.25/0.5/0.75, skipping")
                return
            W[:, idx_mh] = 0.0
    ...
    # Create model
    model = make_single_mvau_model(W, numInputVectors, pe, simd, m, wdt, idt, odt, T, tdt, mem_mode, ram_style, ram_style_thr)
    ...
    if do_synthesis:
        ...
        start_test_batch_fast(results_path=results_dir_power,
                              project_path=os.path.join(ooc_synth_results["vivado_proj_folder"], "vivadocompile", "vivadocompile.xpr"),
                              run_target="impl_1", pairs=[(25, 0.5), (50, 0.5), (75, 0.5)])
    ...
    if do_sim_power:
        ...
        sim_power_report(results_path=results_dir_power,
                         project_path=os.path.join(ooc_synth_results["vivado_proj_folder"], "vivadocompile", "vivadocompile.xpr"),
                         in_width=inst.get_instream_width(), out_width=inst.get_outstream_width(), dtype_width=idt.bitwidth(),
    ...
    if do_synth_power:
        ...
        model = model.transform(CreateStitchedIP("xczu28dr-ffvg1517-2-e", 5))
        model = model.transform(MakeZYNQHarnessProject(platform="RFSoC2x2", results_dir=results_dir, run_id=run_id, dut_duplication=dut_duplication))
