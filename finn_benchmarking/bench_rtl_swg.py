...
def make_single_slidingwindow_modelwrapper(
    type, k, ifm_ch, ifm_dim, ofm_dim, simd, m, parallel_window, stride, dilation, idt, dw=0, ram_style="auto"
):
    k_h, k_w = k
    ifm_dim_h, ifm_dim_w = ifm_dim
    stride_h, stride_w = stride
    ...
    if synthesize_hls_comparison:
        ...
        if not is_square or dilation[0] != 1 or dilation[1] != 1:
            ...
            if parallel_window > 0:
                ...
                if not (fully_unfolded and non_dws and no_stride and no_dilation and supported_ram_style):
                    output_dict["HLS_compatible"] = "no"
