...
import importlib.util                           #to import module from specific path
import sys                                      #to import module from specific path
...
class PAF:
    ...
    def __init__(self, board):
        ...
        if not board in PAF.supported_boards:
            raise ValueError(f"Board '{board}' not supported yet. Please choose one of the supported boards: {PAF.supported_boards}. Alternatively implement the needed dirver.")

        _ffi = FFI()
        _ffi.cdef("""struct sensor {
            char* name;
            int id;
            char* unit;
            double value;
        ...
        board_driver_path = os.path.abspath(f"{framework_dirname}/../{board}/{board}.so")
        if os.path.isfile(board_driver_path):
            PAF._lib = _ffi.dlopen(board_driver_path)
        else:
            raise FileNotFoundError(f"Board driver not found at {board_driver_path}. Please ensure the file exists by running 'make shared' in the driver folder.")
...
class Recorder:
    ...
    def __init__(self, rails=[], sensors=[], power_supply_ip=None, interval=1.0, num_datapoints=-1, omit_voltage=False, omit_current=False, omit_group_elements=False):
        ...
        self._rails = []
        self._sensors = []
        self._power_supply = PowerSupply(power_supply_ip) if not power_supply_ip == None else None
        self._num_datapoints = num_datapoints
        self._df_rails, self._df_sensors = self._prepare_df(rails, sensors, self._power_supply)
        self._stopped = Event()
        self._thread = Thread(target=self._thread_record_data, args=[self._rails, self._sensors, self._power_supply])
        self._running = False
    ...
    def _prepare_rail_group(self, group):
        ...
        for rail in group_rails:
            ...
            if self._omit_group_elements == False:
                columns = columns + self._prepare_rail(rail, False)
        self._rails.append(rail_group) #FIXME no need to append here. Remove this and add_to_rails from _prepare rail, just let _prepare_rail add the rails
    ...
    def _prepare_rail(self, rail, add_to_rails=True):
        ...
        """
        columns = []
        if self._omit_voltage == False: columns.append(rail + "_voltage")
        if self._omit_current == False: columns.append(rail + "_current")
    ...
    def _prepare_df(self, rails, sensors, power_supply):
        ...
        if not power_supply == None:
            if self._omit_voltage == False: rails_columns.append("power_supply_voltage")
            if self._omit_current == False: rails_columns.append("power_supply_current")
    ...
    def save_dfs_to_xlsx(self, title):
        ...
        with pd.ExcelWriter(f"{title}.xlsx") as writer:
                self._df_rails.to_excel(writer, sheet_name=f"rails")
                self._df_sensors.to_excel(writer, sheet_name=f"sensors")
    ...
    def _update_group(self, rail_group):
        ...
        for rail in rail_group:
            rail.update()
            if self._omit_group_elements == False:
                if self._omit_voltage == False: measurements.append(rail.voltage)
                if self._omit_current == False: measurements.append(rail.current)
        ...
        if len(rails) > 0:
            ...
            for element in rails:
                if type(element) == list:
                    ...
                    element.update()
                    if self._omit_voltage == False: rail_measurements.append(element.voltage)
                    if self._omit_current == False: rail_measurements.append(element.current)
                    rail_measurements.append(element.power)
                    total_power += element.power
            rail_measurements.append(total_power)

            if not power_supply == None:
                if self._omit_voltage == False: rail_measurements.append(power_supply.measure_voltage("CH1"))
                if self._omit_current == False: rail_measurements.append(power_supply.measure_current("CH1"))
...
class PowerSupply:
    ...
    def set_channel_settings(self, channel, voltage, current):
        ...
        if channel == "CH1" and voltage == "12.2" and current == "10":
            self._instrument.write(f":OUTP {channel}, OFF") #turn off output before setting new values
            time.sleep(1)
            self._instrument.write(f":APPL {channel},{voltage},{current}")
            time.sleep(1)
            self._instrument.write(f":OUTP {channel}, ON") #turn channel on again
        else:
            print("ERROR: Trying to set something other than CH1 to 12.2V @ 10A is not supported!!!") #TODO change this if other settings should be supported
...
class LiveVisualization:
    ...
    def __init__(self, names):
        ...
        names : list
            ...
            set the plot title. 
        """
        subplots = self._build_subplots(names[1:]) #first entry in names is plot title, the rest are trace names
...
class Experiment:
    ...
    def __init__(self, config=None, **kwargs):
        ...
        if not config == None:
            ...
            self._end_after = config.get("end_after", None) #end experiment after set seconds, None indicates no timeout 
    ...
    def _setup_experiment(self):
        ...
        for path in self._import_paths:
            if path not in sys.path:
                ...
                print(f"Path: {path} already part of sys.path. Removing from experiment import paths")
        ...
        self._recorder = Recorder(self._rails, self._sensors, self._power_supply_ip, omit_current=True, omit_voltage=True)
    ...
    def start_experiment(self):
        ...
        for i in range(1, self._iterations+1):
            ...
            for function in self._config["functions"]:
                ...
                kwargs["bitstream_path"] = self._config["FINN"]["bitstream"]

                print(f"Calling func_name {name} with args: {args}\n kwargs {kwargs}")
                func = getattr(self._experiment_module, name)

                if(self._end_after == None):
                    #func(*args, **kwargs)
                    # execute asynchronously to implement proper warmup
                    p = multiprocessing.Process(target=func, args=args, kwargs=kwargs)
                    p.start()
                else:
                    print("ERROR, DO NOT USE THIS FEATURE") #TODO: fix for fixed warmup, what if multiple functions are defined?
                    p = multiprocessing.Process(target=func, args=args, kwargs=kwargs) 
                    p.start()
            ...
            self._recorder.stop()
            self._recorder.save_dfs_to_xlsx(f"{self._title}_run{i}")
...
class ExperimentFlow:
    ...
    def _append_global(self, experiment_config):
        ...
        for gk in self._config["global"].keys():
            if not gk in experiment_config.keys():
